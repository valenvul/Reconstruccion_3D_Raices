...
from disparity.methods import Config, EnumParameter, StereoMethod, InputPair, StereoOutput
...
class RaftStereo(StereoMethod):
    ...
    def reset_defaults(self):
        self.parameters.update({
            ...
            "Model": EnumParameter("Pre-trained Model", 0,
                                   ["fast-cpu", "middlebury-cpu", "eth3d-cpu", "fast-cuda", "middlebury-cuda",
                                    "eth3d-cuda"])
        })
    ...
    def _compute_disparity(self, input: InputPair) -> StereoOutput:
        ...
        if disparity_map.shape[:2] != input.left_image.shape[:2]:
            disparity_map = cv2.resize(disparity_map, (input.left_image.shape[1], input.left_image.shape[0]),
                                       cv2.INTER_NEAREST)
